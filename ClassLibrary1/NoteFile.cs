    ...
    class NoteFile
    {
        ...
        public static bool SaveNotes(string path, params Note[] notes)
        {
            ...
            if (notes.Where(x => x != null).Count() == notes.Length)
            {
                try
                {
                    ...
                    notes.ToList().ForEach(x => File.AppendAllText(path, $"{x.Title}>{x.Text}\n<{x.CreationData.Date}|\n\n*"));
                    res = true;
                }
                catch { }
            }
        ...
        public static List<Note> ReadFromFile(string path)
        {
            ...
            if (path != null && File.Exists(path))
            {
                ...
                data.Split('*').ToList().ForEach(x => 
                { 
                    if (x.Count() > 5)  
                        notesFromFile.Add(new Note(new string(x.TakeWhile(y => y != '>').ToArray()), 
                            x.Substring(x.LastIndexOf('>') + 1, 
                            x.LastIndexOf('<') - x.LastIndexOf('>') - 1), 
                            DateTime.Parse(x.Substring(x.IndexOf('<') + 1, 
                            x.LastIndexOf('|') - x.LastIndexOf('<') - 1)))); 
                });
            }
            else
